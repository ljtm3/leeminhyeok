...
public class HomeController {
	 ...
	 * 그래서, 저장시 보시면, Reloading Context with... is completed 재실행 되었다고 나옵니다.
	...
	public String weare() {
		//외부에서 /weare경로로 접근했을때, sample/weare.jsp와 매킹시키라는 의미.
		return "sample/weare";
	}
	...
	public String index()  {//메서드(함수)구성:리턴(반환값출력)형태지정 메서드명(매개변수) {구현내용} string index(){}
		...
		//여기서는 index.jsp와 매칭되는 문자열을 반한. 상대경로가 필요합니다.
	...
	public String home(Locale locale, Model model) {
		...
		System.out.println("함수-C언어나자바스크립트-와 매서드-자바,스프링에서 사용함-는 같은 대상을 가리킵니다");
		System.out.println("함수-메서드는 함수명('매개변수-입력값'){구현내용}형식이고, 입력값->출력값 구현됩니다.");
		Date date = new Date();//Date 날짜 관련 클래스형 변수 date 선언했습니다. date변수 메서를 사용 가능하게됨
		//data 변수 실행가능한 변수=클래스형변수=Object오브젝트=인스턴스 라고 합니다.(위)
		DateFormat dateFormat = DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG, locale);
		//DataFormat 클래스형 변수 dateFormat 이 선언되었고, 실행가능한 변수로 되었습니다.(위)

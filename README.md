#### 20201119(목) 작업예정
*, :after, :befor { /* :콜론의 의미 가상선택자
     -webkit-box-sizing: border-box;/* 크롬, 사파이룡

- index.html 더미데이터에 모바일용 CSS적용예정.
- html(jsp)영역지정방식:tag방식, class방식, id방식
- id방식은 현재html문서에서 1개만 존재해야함.(로그인시 사용하는 아이디와 같은 기능)
- class방식은 현재html문서에서 여러게 존재해도 가능함.
- reset.css 를 불러오게 될 예정 입니다. 이 역할은 아래
- 크로스브라우징 사전처리 역할(아래)
- IE, 크롬, 파이어폭스, 사파리(Sapari) 다양한 웹브라우저 콘텐츠가 같은크기로 보이게 하는 역할
============================================
- 메뉴 클릭에 대한 자바스크립트적용.
...
=========================================================
- edu데이터베이스에 address테이블 만들었습니다.(SQL 타이핑키보드로)
1. 과제물제출 50점
2. 서술형시험 50점
- 합계 100으로 평가가되고,
1. 과제물제출은 구글 문서에 있는 내용입니다.
2. 서술형 시험은 sw기초기술활용과목 1,2,3단원에서, 각3문제총 9문제가 나오게 됩니다.
- 대부분 단답형이고, SQL쿼리도 2개 문제 포함 되어 있습니다.
- SQL쿼리는 ERD테이블그림을 보시고, 테이블 생성하는 쿼리를 만드시고, 제출
- 입력결과 예시를 보시고, insert쿼리도 1개 만드는 내용도 있습니다.
- egov실행, 만세아이콘(wamp)실행,

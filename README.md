#### 20201113(금)
- IT분야는 1번 했냐 안했냐가 중요#
- Java(자바) : 오라클에서 제작한(라이센스가 있는) 프로그램밍 언어.
- 자바로 만든 홈페이지가 jsp 입니다.
- jsp(Java Server Pgae): 스프링MVC프로젝트 신규 생성(새문서생성)시 아래기본출력
- 꺽쇠<%....%>: 조건은 jsp파일에서 자바프로그램을 사용할때 <%자바프로그램%>
- <% taglib 애노테이션 페이지 지시자(어) 역할 : 예, 페이지 인코딩(한글처리)
- taglib = url("외부자바프로그램용 태그를 가져다가 사용할 주소")
- prefix(접두사-축약어) = "c'
- <%@ page session 세션 : 로그인 상태를 유지할때 사용합니다.
- @ 애노테이션 페이지 지시자(어) 역할:페이지 인코딩(한글처리)
- 서버시간 톰캣서버의 시간(이클립스컴퓨터시간), 헤로쿠올리면(헤로쿠서버의 시간이 나옴)
...
2. 파스타에서는 Mysql 데이터베이스 사용.(서버를 껐다가 켜면, 내용이 원상복귀)
- 파스타는 다음주 월요일 같이 진행하도록 하겠습니다.
- IDE : 통합개발환경 Integeration Development Environment(이클립스, 안드로이드스튜디오)
MVC : 자바(스프링)개발 방식중에 3가지 분리해서 개발한느 방식 Model, view, controller
- Vieww : home.jsp 
- 앞으로 여러분들의 작업순서가 아래처럼진행됩니다.
...
- war(와르파일)
- 깃에 올릴때, 제외시킬 폴터를 지정. target폴더는 깃에서 제외시키는 방법
- .gitignore 파일을 만들어서. target 이름을 적으시면 됩니다.
- root루트 : 최상위 위치를 가리킵니다.
- POM.xml 파일에서 버전을 변경하게 됩니다.
- Program 에 사용되는 Object Module(외부프로그램들=모듈) 관리한는 파일을 pom.xml
- 스프링-메이븐 프로젝트이면, 필수로 생성되는 파일 pom.xml입니다.
- pome.xml에 있는 의존성(dependency)은 외부프로그램모듈에 대한 의존해서 갖다가 사용한다.
- 서불렛(serclet) : server프로그램+let(조각), 미니서버프로그램
- 인젝트(Inject-주사바늘삽입) : 외부클래스 사용할때 쓰는 지시어 @Inject 사용
- Aspect(관점) : 미술에서 사용하던 용어. @Aspect 사용.
- 지시어 = 애노테이션 = @
- 이클립스 file메뉴> 제일하단 리스타트클릭
- 자바버전 1.6 -> 1.8 변경
- 깃서버와 이클립스 소스 연동
- 깃 저장 순서 : 커밋(.git폴덜에 저장) + 푸시(github.com에저 저장)
- 형상관리 = 버전관리 = 소스관리 = 팀작업
- 이클립스 사용법 :
- 화면 왼쪽 상단 : Package Explore 패키지탐색기로 보이게 됩니다.
- 퍼스펙티브(Perspective) : 개발자 관점. (java, eGov개발자관점)
- 만약에 관점이 이상하게 변해있다면, window메뉴>perspective>Reset~클릭으로 해결
